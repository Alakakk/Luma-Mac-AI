...
if (process.platform === 'win32') {
  ...
  app.setPath('userData', path.join(app.getPath('appData'), DEMO_MODE ? 'luma-todo-mac-trial-demo' : 'luma-todo-mac-trial'));
} else if (DEMO_MODE) {
...
function createWindow() {
  ...
  mainWindow.once('ready-to-show', async () => {
    ...
    if (process.env.LUMA_SCREENSHOT_DIR) {
      ...
      if (!app.isPackaged && DEMO_MODE && process.env.LUMA_AI_SMOKE === '1') {
        try {
          const result = await mainWindow.webContents.executeJavaScript(fs.readFileSync(path.join(__dirname, 'qa/ai-ui-smoke.js'), 'utf8'));
          console.log('[Luma Todo] AI UI smoke: ' + result.join(', '));
...
app.whenReady().then(() => {
  ...
  if (process.platform === 'darwin') {
    Menu.setApplicationMenu(Menu.buildFromTemplate([
      ...
      { label: '窗口', submenu: [{ label: '关闭窗口', accelerator: 'Cmd+W', click: () => mainWindow.close() }] },
    ]));
  }
  setupAutoUpdates();
});

app.on('activate', () => { if (mainWindow && !mainWindow.isDestroyed()) revealMainWindow(); });
...
trustedHandle('window:set-expanded', (_event, expanded) => {
  ...
  const size = {
    width: (remembered?.width || fallback.width) + (isAiOpen ? 350 : 0),
  ...
  mainWindow.setMinimumSize(isAiOpen ? 650 : (isExpanded ? 760 : 330), 420);
...
trustedOn('window:resize-move', (_event, payload) => {
  ...
  const minWidth = isAiOpen ? 650 : (isExpanded ? 760 : 330);
  const minHeight = 420;
  const next = { ...bounds };
...
require('./main/ai.cjs')({ app, safeStorage, handle: trustedHandle, Menu });
trustedHandle('window:ai-panel', (_event, enabled) => {
  const next = Boolean(enabled);
  if (next === isAiOpen) return next;
  const bounds = mainWindow.getBounds();
  const area = screen.getDisplayMatching(bounds).workArea;
  const width = Math.min(area.width - 40, Math.max(next ? 650 : (isExpanded ? 760 : 330), bounds.width + (next ? 350 : -350)));
  isAiOpen = next;
  mainWindow.setMinimumSize(next ? 650 : (isExpanded ? 760 : 330), 420);
  mainWindow.setBounds({ ...bounds, width, x: Math.max(area.x + 20, Math.min(bounds.x, area.x + area.width - width - 20)) });
  return next;
});
