(async () => {
  ...
  const savedState = await window.luma.load();
  assertQa(savedState.tasks.some((item) => item.id === task.id && item.time === '09:30'), 'Saved Todo did not survive reading from disk');
